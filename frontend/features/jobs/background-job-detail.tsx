  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 text-sm md:grid-cols-4">
        <div><span className="text-zinc-500">Loại</span><p className="font-semibold">{backgroundJobTypeLabel(job.type)}</p><p className="mt-1 font-mono text-xs text-muted">{job.type}</p></div>
        <div><span className="text-zinc-500">Worker status</span><p className="mt-1"><Badge tone={status.tone}>{status.label}</Badge></p></div>
        <div><span className="text-zinc-500">Lần thử</span><p className="font-mono font-semibold tabular-nums">{job.attempts}/{job.max_attempts}</p></div>
        <div><span className="text-zinc-500">Lần kiểm tra</span><p className="font-mono font-semibold tabular-nums">{job.poll_attempts}/{job.max_poll_attempts}</p></div>
      </div>
      {job.invoice_job_id && (
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4">
          <p className="mb-1 text-xs font-medium uppercase tracking-widest text-emerald-600">Tác vụ hóa đơn</p>
      ...
      {job.last_error_json ? (
        <div>
          <p className="mb-2 text-sm font-semibold text-red-700">Lỗi gần nhất (đã che dữ liệu nhạy cảm)</p>
          <pre className="max-h-48 overflow-auto rounded-xl bg-zinc-950 p-3 text-xs text-red-300">{maskError(job.last_error_json)}</pre>
        </div>
      ) : null}
      <div>
        <p className="mb-2 text-sm font-semibold text-zinc-700">Dữ liệu kỹ thuật</p>
